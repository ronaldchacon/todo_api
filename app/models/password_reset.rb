class PasswordReset
  ...
  attr_accessor :email, :reset_password_redirect_url, :reset_token, :password,
                :password_confirmation, :updating
  ...
  def update
    self.updating = true
    user && valid? && user.complete_password_reset(password, password_confirmation)
  end
